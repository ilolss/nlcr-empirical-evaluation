...
## Воспроизводимость и время выполнения

Все экспериментальные ноутбуки используют единый `SEED = 42` для Python,
NumPy и стохастических моделей. Ноутбуки сохраняют wall-clock замеры отдельно
для базового прогноза и NLCR-проекции:

- `results/tables/*_timing.csv` — отдельные эксперименты;
- `results/tables/summary_timing.csv` — сводная таблица;
- `simulations/simulation_timing.csv` — Monte Carlo-симуляции.

Время `total_prediction_seconds` соответствует `baseline + reconciliation`.
Абсолютные значения зависят от оборудования, поэтому для сравнения также
сохраняются миллисекунды на прогноз и `slowdown_vs_base`.
